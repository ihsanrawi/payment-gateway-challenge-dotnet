...
public class PaymentProcessorServiceTests
{
    ...
    [Fact]
    public async Task ProcessRejectedPaymentAsync_DoesNotStoreIdempotencyMapping_AllowsRetry()
    {
        // Arrange
        var idempotencyKey = Guid.NewGuid();
        var request = CreateValidRequest();
        ...
        // Act
        var result = await _sut.ProcessRejectedPaymentAsync(idempotencyKey, request);

        // Assert
        _paymentsRepositoryMock.Verify(x => x.Add(It.Is<Payment>(p => p.Status == PaymentStatus.Rejected)), Times.Once);
        // IMPORTANT: Idempotency mapping should NOT be stored for rejected payments
        _idempotencyRepositoryMock.Verify(x => x.StoreMapping(It.IsAny<Guid>(), It.IsAny<Guid>()), Times.Never);
        _idempotencyRepositoryMock.Verify(x => x.GetPaymentId(It.IsAny<Guid>()), Times.Never);
        Assert.Equal(PaymentStatus.Rejected, result.Status);
    }

    [Fact]
    public async Task ProcessRejectedPaymentAsync_DoesNotCheckExistingIdempotencyKey()
    {
        // Arrange - This verifies that rejected payments don't check existing idempotency mappings
        var idempotencyKey = Guid.NewGuid();
        var existingPaymentId = Guid.NewGuid();
        var request = CreateValidRequest();

        // Setup an existing idempotency mapping (should be ignored for rejected payments)
        _idempotencyRepositoryMock
            .Setup(x => x.GetPaymentId(idempotencyKey))
            .Returns(existingPaymentId);
        ...
        // Assert - Should create a new rejected payment, NOT return the existing one
        _idempotencyRepositoryMock.Verify(x => x.GetPaymentId(It.IsAny<Guid>()), Times.Never);
        _paymentsRepositoryMock.Verify(x => x.Get(It.IsAny<Guid>()), Times.Never);
        _paymentsRepositoryMock.Verify(x => x.Add(It.IsAny<Payment>()), Times.Once);
        Assert.Equal(PaymentStatus.Rejected, result.Status);
    }
}